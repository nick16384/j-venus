...
public class Lib {
	...
	public static void initVars() {
		if (System.getProperty("java.home") == null) {
			...
			System.err.println("INITVARS: JavaDOS needs JDK installed in order to work. Exiting...");
		...
		if (osName.equalsIgnoreCase("Linux")) { //If operating system is linux...
			fsRoot = "/";
			jDOSDefDir = new File(fsep + "etc" + fsep + "javados");
			//jDOSDefDir = new File(fsep + "home" + fsep + "theophil" + fsep + "Desktop" + fsep + "JavaDOS");
			...
			path = "/";
			try { consoleLogStream = new PrintStream(new File("/home/theophil/Desktop/log1.txt")); }
			catch (FileNotFoundException fnfe) {}
			catch (NullPointerException npe) {}
		} else if (osName.contains("Windows")) { //If operating system is any Windows version...
			fsRoot = System.getenv("SYSTEMROOT") + "\\";
			jDOSDefDir = new File("C:" + fsep + "Program Files" + fsep + "JavaDOS");
			jDOSTempDir = new File(jDOSDefDir.getAbsolutePath() + fsep + "temp");
			jDOSBinDir = new File(jDOSDefDir.getAbsolutePath() + fsep + "bin");
			jDOSCmdDir = new File(jDOSDefDir.getAbsolutePath() + fsep + "commands");
			path = System.getenv("SYSTEMROOT") + "\\";
			try { consoleLogStream = new PrintStream(new File(System.getenv("USERPROFILE" + fsep + "Desktop" + fsep + "log1.txt"))); }
			catch (FileNotFoundException fnfe) {}
			catch (NullPointerException npe) {}
		}
	...
	public static void cmdLinePrepare(boolean InitPhase) {
		...
		if (InitPhase) {
			...
			HighLevel.shell_write(1, "HIDDEN", "Copyright(C) 2021 - 2022 The JavaDOS Project. All rights reserved. \n");
			...
			//HighLevel.shell_write(1, "HIDDEN", "Set Log Folder: " + jDOSLogDir.getAbsolutePath() + "\n");
			HighLevel.shell_write(1, "HIDDEN", "\n");
			HighLevel.shell_write(1, "HIDDEN", "Running in shell mode. Type 'end-session' to return to linux. \n");
			HighLevel.shell_write(1, "HIDDEN", "\n");
	...
	public static void logWrite(String auth, int status, String message) {
		...
		if (consoleLogStream != null) {
			if (status == -1) {
				consoleLogStream.println("[ " + getDateTime(false) + ", -1/ERR, " + auth + " ]: " + message);
			} else if (status == 0) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + auth + " ]: " + message);
			} else if (status == 1) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + "1/INFO, " + auth + " ]: " + message);
			} else if (status == 2) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + "2/WARN, " + auth + " ]: " + message);
			} else if (status == 3) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + "3/NONCRIT, " + auth + " ]: " + message);
			} else if (status == 4) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + "4/CRIT, " + auth + " ]: " + message.toUpperCase());
			} else if (status == 5) {
				consoleLogStream.println("[ " + getDateTime(false) + ", " + "5/FATAL, " + auth +  " ]: " + message.toUpperCase());
			}
		}
	}
