...
public class Cipher_Encrypt {
   public static String encrypt(ArrayList<String> params, Map<String, String> paramsWithValues) {
     if (params == null || paramsWithValues == null || params.size() < 1 || paramsWithValues.size() != 2) {
       sys.log("ENCRYPT", LogLevel.ERR, "reqParam Parse error");
    ...
    catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Error reading input file, aborting."); return "FileRWError"; }
    
    if (params.contains("shift2c")) {
      
      sys.log("ENCRYPT", LogLevel.STATUS, "Encryption with Shift2C started...");
      sys.log("ENCRYPT", LogLevel.DEBUG, "Text to encrypt: " + plaintext);
      String key = "";
      //SecureRandom random = new SecureRandom();
      plaintext = plaintext.replaceAll(" ", "E");
      plaintext = plaintext.toUpperCase();
      
      sys.log("ENCRYPT", LogLevel.INFO, "Using given key.");
      key = paramsWithValues.get("key");

      String shiftlist = "";
      Map<String, String> shiftOrder = new HashMap<>();
      Map<String, String> shiftOrderAfter = new HashMap<>();
      sys.log("ENCRYPT", LogLevel.STATUS, "Reading SHIFTLIST.txt to assign shift order"); 
      ...
      catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Error reading SHIFTLIST.txt, aborting"); return null; }
       sys.log("ENCRYPT", LogLevel.ERR, shiftlist);
      ...
      for (i = (arrayOfString1 = splittedShiftlist[0].trim().split("\n")).length, b1 = 0; b1 < i; ) { String value = arrayOfString1[b1];
       ...
       sys.log("ENCRYPT", LogLevel.STATUS, "Reading shiftlist done.");
      
      sys.log("ENCRYPT", LogLevel.DEBUG, "Encrypting...");
      sys.log("ENCRYPT", LogLevel.DEBUG, "Prepared text to encrypt " + plaintext);
      ...
      for (j = (arrayOfString2 = plaintextChars).length, b2 = 0; b2 < j; ) { String value = arrayOfString2[b2];
        for (int k = 0; k < Integer.parseInt(Character.toString(key.charAt(ciphertext.length()))); k++) {
          if (shiftOrder.get(value) != null) {
            sys.log("ENCRYPT", LogLevel.DEBUG, "Encryption, Stage 1, Char " + value + ", Round " + k);
            value = value.replace(value, shiftOrder.get(value));
            value = value.trim();
            if (shiftOrderAfter.get(value) != null) {
              sys.log("ENCRYPT", LogLevel.DEBUG, "Encryption, Stage 2, Char " + value + ", Round " + k);
              value = value.replace(value, shiftOrderAfter.get(value));
              value = value.trim();
            } 
          } else {
            
            sys.log("ENCRYPT", LogLevel.WARN, "Warning: Character: " + value + " not present in SHIFTLIST.txt");
          } 
        } 
        ciphertext = ciphertext.concat(value); b2++; }
      
      if (ciphertext.length() == key.length() && !ciphertext.equalsIgnoreCase(plaintext)) {
        sys.log("ENCRYPT", LogLevel.INFO, "Encryption successful.");
      } else {
        sys.log("ENCRYPT", LogLevel.ERR, "Encryption failed.");
      } 
      
      if (ciphertext.length() < 100) {
        sys.log("ENCRYPT", LogLevel.INFO, "Output/Ciphertext: " + ciphertext);
        sys.log("ENCRYPT", LogLevel.INFO, "Key: " + key);
      }
      sys.log("ENCRYPT", LogLevel.INFO, "Saving files to 'key.txt' and 'out.txt'");
      ...
      try { outFile.createNewFile(); } catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Could not create output file."); }
       try { keyFile.createNewFile(); } catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Could not create key file."); }
       try { Files.writeString(outFile.toPath(), ciphertext, new OpenOption[] { StandardOpenOption.APPEND }); }
      catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Could not write to output file."); }
       try { Files.writeString(keyFile.toPath(), key, new OpenOption[] { StandardOpenOption.APPEND }); }
      catch (IOException ioe) { sys.log("ENCRYPT", LogLevel.ERR, "Could not write to key file."); }
       sys.log("ENCRYPT", LogLevel.DEBUG, "Done.");
    } 
