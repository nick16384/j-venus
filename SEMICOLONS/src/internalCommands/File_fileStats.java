...
public class File_fileStats {
	public static String fileStats (ArrayList<String> params, Map<String, String> paramsWithValues) {
		...
		if (!new ParameterChecker(params).checkValid()) {
			sys.log("FSTAT", LogLevel.STATUS, "No parameter given, reading working directory permission.");
		...
		//Parameters are valid, check if element exists globally
			...
			sys.log("FSTAT", LogLevel.DEBUG, "File \"" + params.get(0) + "\" exists at root level.");
		...
		//Check if element exists locally (inside working directory)
			...
			sys.log("FSTAT", LogLevel.DEBUG, "File \"" + params.get(0) +  "\" exists inside working directory as \""
		...
		//Supplied file / folder name not valid
			...
			sys.log("FSTAT", LogLevel.ERR, "Specified file \"" + params.get(0) + "\" does not exist.");
