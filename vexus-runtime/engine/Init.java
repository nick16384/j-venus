...
public class Init {
	/**
	 * Initializes the J-Vexus environment.
	...
	public static boolean init(String[] vmArgsArray) {
		...
		if (vmArgs.contains("--help") || vmArgs.contains("-h")) {
			...
			System.out.println("J-Vexus version " + VarLib.getVersion());
			System.out.println("Usage: vexus [OPTION...] [STARTDIR]");
			System.out.println("Info: [STARTDIR] not implemented yet.");
			System.out.println("See list below for all arguments:\n");
			System.out.println("View:");
			System.out.println("\t--fullscreen, --full-screen \t Starts J-Vexus in fullscreen mode (no window)\n");
		...
		if (vmArgs.contains("--single-threaded")) {
			...
			System.out.println("or reinstalling J-Vexus. If it's a general problem, please report it.");
		...
		if (!main.Main.singleThreaded) { modules.WatchdogThread.runThreads(); }
		try {
		...
		sys.log("MAIN", 1, "Finished startup.");
