...
public class CommandMain {
	...
	private static boolean allowParallelExecution = false;
	...
	public static void terminate() {
		try {
			...
			if (commandType.equals("internal")) {
				...
				sys.shellPrintln(ANSI.D_Magenta, "Thread interrupt.");
	...
	public static String executeCommand(components.Command cmd) throws IOException {
		try {
			...
			optionsWithValues = cmd.getOptionsWithValues();
			params = cmd.getParams();
			//TODO if cmdQueue is not empty, set noPrompt to true (inside execThread)
			//TODO run sudo do-release-upgrade on theophil-Ubuntu
		...
		if (params != null && params.contains("-allowParallelExecution")) allowParallelExecution = true;
		
		if (!execThread.isAlive() || (allowParallelExecution)) {
		...
		try {
			if (!sys.getCurrentShellMode().equalsIgnoreCase("normal")) {
				...
				if (command.equalsIgnoreCase("encrypt")) {
					...
					commandType = "system";
					returnVal = internalCommands.System_Exec.sysexec(commandObj);
				} else {
					//Command is not internal (external, binary or not existent)
					commandType = "external";
					sys.shellPrintln(ANSI.B_Blue, "Extern");
					...
					if (new File(commandObj.getCommand()).isFile()) {
						sys.shellPrintln(ANSI.B_Blue, "Extern : fullPath");
						...
						sys.shellPrintln(ANSI.B_Blue, "Extern : fullPathWithoutJar");
						if (commandObj.getFullCommand().contains(" ")) {
							sys.shellPrintln(ANSI.B_Blue, "Extern : fullPathWithoutJar : params");
							execProcBuild = new ProcessBuilder(VarLib.getJavaExec().getAbsolutePath(),
									"-jar", commandObj.getFullCommand().replaceFirst(" ", ".jar "));
						} else {
							sys.shellPrintln(ANSI.B_Blue, "Extern : fullPathWithoutJar : noParams");
							execProcBuild = new ProcessBuilder(VarLib.getJavaExec().getAbsolutePath(),
						...
						sys.shellPrintln(ANSI.B_Blue, "Extern : noPath");
						execProcBuild = new ProcessBuilder(VarLib.getJavaExec().getAbsolutePath(),
								"-jar", VarLib.getCmdDir().getAbsolutePath() + VarLib.fsep + commandObj.getFullCommand());
					} else if (new File(VarLib.getCmdDir().getAbsolutePath() + VarLib.fsep
							+ commandObj.getCommand() + ".jar").isFile()) {
						sys.shellPrintln(ANSI.B_Blue, "Extern : noPathWithoutJar");
						if (commandObj.getFullCommand().contains(" ")) {
							sys.shellPrintln(ANSI.B_Blue, "Extern : noPathWithoutJar : params");
							execProcBuild = new ProcessBuilder(VarLib.getJavaExec().getAbsolutePath(),
									"-jar", VarLib.getCmdDir().getAbsolutePath()
									+ VarLib.fsep + commandObj.getFullCommand().replaceFirst(" ", ".jar "));
						} else {
							sys.shellPrintln(ANSI.B_Blue, "Extern : noPathWithoutJar : noParams");
							execProcBuild = new ProcessBuilder(VarLib.getJavaExec().getAbsolutePath(),
									"-jar", VarLib.getCmdDir().getAbsolutePath()
					...
					if (commandType.equals("external")) {
						try {
							sys.shellPrintln(ANSI.D_Cyan, "----------------------------------------------------------");
							sys.shellPrintln(ANSI.D_Cyan, "Debug information:");
					...
					if (execProc == null || !execProc.isAlive() || execProc.info().toString().equals("[]")) {
						...
						commandType = "system";
						sys.shellPrintln(ANSI.B_Blue, "System");
