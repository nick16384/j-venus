...
public class VarLib {
	protected static final String VERSION = "22.10";
	//Message of the day -> start msg before any commands e.g. Welcome to ...
	protected static String motd = "J-Vexus Version " + VERSION + " Warning: Beta state, problems may occur.\n"
			+ "Copyleft (🄯) 2021 - 2022 The J-Vexus Project. All rights reserved.\n"
	...
	protected static final String DEFAULT_PROMPT_PATTERN = "$USERNAME@$HOSTNAME:$PATH$# ";
	...
	public static void setPromptPattern(String newPromptPattern) {
		if (newPromptPattern != null) {
			...
			sys.log("OPENLIB", 2, "Tried to set prompt to null, changing nothing.");
		}
	}
	...
	public static void addEnv(String key, String val) {
		...
		if (getEnv(key) != null && getEnv(key).equals(val)) {
			...
			sys.shellPrint(ANSI.B_Green, "Success: " + key + " -> " + val + "\n");
		} else {
			sys.log("VARLIB", 3, "Validation failed; " + val + " != " + getEnv(key));
			sys.shellPrint(ANSI.B_Yellow, "Could not create envV. Information below:\n");
			sys.shellPrint("Created variable, but validation failed \\/\n"
					+ "Method call key: " + key + "\n"
					+ "Method call value: " + val + "\n"
					+ "envV Value with getEnv() call: " + getEnv(key) + "\n");
			sys.shellPrint(ANSI.B_Magenta, "Try 'env' to see, if your envV exists or try again.\n");
		}
	}
	...
	private static int getCmdLineLineNumber() {
		...
		Element root = main.Main.cmdLine.getDocument().getDefaultRootElement();
	...
	public static void fetchMOTD() {
		...
		if (motdRaw != "") {
			...
			while (motd.contains("$")) {
				evcount++;
				try {
					//TODO this throws NPE, probably because of envV not found or REGEX wrong syntax.
					motd = motd.replaceFirst(
							"\\$[A-Z]+",
							getEnv(motd.substring(motd.indexOf("$")).split(" ")[0]));
				} catch (NullPointerException npe) {
