...
public class OpenLib {
	public static void initVars() {
		if (System.getProperty("java.home") == null) {
			...
			new modules.WatchdogThread().shutdownVexus(1);
		}
		...
		if (VarLib.osName.equalsIgnoreCase("Linux")) { //If operating system is linux...
			VarLib.fsRoot = "/";
			VarLib.DfltDir = new File(VarLib.fsep + "etc" + VarLib.fsep + "vexus");
			...
			VarLib.DfltDir = new File("C:" + VarLib.fsep + "Program Files" + VarLib.fsep + "J-Vexus");
			VarLib.TempDir = new File(VarLib.DfltDir.getAbsolutePath() + VarLib.fsep + "temp");
			VarLib.BinDir = new File(VarLib.DfltDir.getAbsolutePath() + VarLib.fsep + "bin");
			VarLib.CmdDir = new File(VarLib.DfltDir.getAbsolutePath() + VarLib.fsep + "commands");
			VarLib.DataDir = new File(VarLib.DfltDir.getAbsolutePath() + VarLib.fsep + "data");
		...
		VarLib.addEnv("$$ALL", "This references all variables, so this text shouldn't be readable.");
		...
		VarLib.addEnv("$VEXUS_ROOT", VarLib.getDefaultDir().getAbsolutePath());
		VarLib.addEnv("$VEXUS_TMP", VarLib.getTempDir().getAbsolutePath());
		VarLib.addEnv("$VEXUS_BIN", VarLib.getBinDir().getAbsolutePath());
		VarLib.addEnv("$VEXUS_CMD", VarLib.getCmdDir().getAbsolutePath());
		VarLib.addEnv("$VEXUS_DATA", VarLib.getDataDir().getAbsolutePath());
	...
	public static void updateEnv(String envName) {
		if (envName.equalsIgnoreCase("$$ALL")) {
			...
			VarLib.changeEnv("$VEXUS_ROOT", VarLib.getDefaultDir().getAbsolutePath());
			VarLib.changeEnv("$VEXUS_TMP", VarLib.getTempDir().getAbsolutePath());
			VarLib.changeEnv("$VEXUS_BIN", VarLib.getBinDir().getAbsolutePath());
			VarLib.changeEnv("$VEXUS_CMD", VarLib.getCmdDir().getAbsolutePath());
			VarLib.changeEnv("$VEXUS_DATA", VarLib.getDataDir().getAbsolutePath());
