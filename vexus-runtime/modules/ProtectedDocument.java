...
public class ProtectedDocument
{
	...
	public void protect(int start, int end)
	{
		try
		{
			if (start == 0 && end == 0) {
				positions.clear();
				//positions.put(doc.createPosition(0), doc.createPosition(0));
				//TODO just the first char in cmdLine is still protected
			} else {
	...
	class ProtectedDocumentFilter extends DocumentFilter
	{
		 ...
		 */
		private boolean isRemoveProtected(int start, int length, boolean... bypass)
		{
